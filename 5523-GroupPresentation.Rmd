...
```{r, echo=FALSE, warning=FALSE, message=FALSE, out.width="100%", fig.height=4.3}
humidity_dat %>%
  ggplot(aes(x = value)) +
...
humidity_dat %>% 
  ...
  geom_rect(aes(xmin = 650, xmax = 1120, ymin = -Inf, ymax = Inf), alpha = 0.01, fill = "#2ECC71") +
  ylim(25, 80) +
...
p1 <- df2 %>% 
  ...
  geom_rect(ymin =30, ymax = 60, xmin = as.Date("2019-11-01", format = "%Y-%m-%d"), 
            xmax = as.Date("2020-11-20", format = "%Y-%m-%d"), fill = "#5dfc87", alpha = 0.005)+
